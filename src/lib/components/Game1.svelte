<script lang="ts">
	import type { Team } from '$lib/types';

	let { teams = [] }: { teams: Team[] } = $props();

	// List of 10 words for the game
	const availableWords = [
		'Apfel',
		'Baum',
		'Schnee',
		'Berg',
		'Skifahren',
		'Hütte',
		'Winter',
		'Freund',
		'Piste',
		'Lift'
	];

	// Game state
	let orderedWords = $state<string[]>(['Sonne ']); // Start with one word
	let remainingWords = $state<string[]>([...availableWords.filter((w) => w !== 'Sonne')]);
	let selectedWord = $state<string>('');
	let selectedPosition = $state<number>(-1);
	let currentTeamIndex = $state<number>(0);

	function selectWord(word: string) {
		selectedWord = word;
	}

	function insertWordAtPosition(position: number) {
		if (selectedWord) {
			// Insert word at selected position
			const newSequence = [...orderedWords];
			newSequence.splice(position, 0, selectedWord);
			orderedWords = newSequence;

			// Remove word from available words
			remainingWords = remainingWords.filter((w) => w !== selectedWord);

			// Reset selection
			selectedWord = '';
			selectedPosition = -1;

			// Next team's turn
			currentTeamIndex = (currentTeamIndex + 1) % teams.length;
		}
	}
</script>

<div class="flex flex-1 p-8" style="background-color: rgba(255, 165, 0, 0.3);">
	<div class="mx-auto w-full max-w-7xl">
		<!-- Header -->
		<div class="mb-4 text-center">
			<h1 class="mb-1 text-4xl font-black text-orange-400">Game 1: Wörter ordnen</h1>
		</div>

		<div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
			<!-- Word Sequence -->
			<div class="space-y-2">
				<h2 class="mb-3 text-2xl font-bold text-orange-300">Wort-Reihenfolge</h2>

				<div class="space-y-1">
					{#each Array.from({ length: orderedWords.length + 1 }, (_, i) => i) as position}
						{#if position < orderedWords.length}
							<!-- Position button -->
							<div class="flex items-center" style="margin-top: -10px;">
								<button
									onclick={() => (selectedWord ? insertWordAtPosition(position) : null)}
									disabled={!selectedWord}
									class="flex h-6 w-6 items-center justify-center rounded-full border-2 text-xs font-bold transition-all {selectedPosition ===
									position
										? 'border-orange-400 bg-orange-600 text-white'
										: selectedWord
											? 'cursor-pointer border-orange-400 bg-orange-500 text-white hover:bg-orange-400'
											: 'cursor-not-allowed border-gray-600 bg-gray-800 text-gray-500 opacity-50'}"
								>
									{position + 1}
								</button>
							</div>

							<!-- Word -->
							<div class="ml-8 flex items-center" style="margin-top: -15px;">
								<div class="flex-1 border border-gray-600 bg-gray-700 p-2 font-semibold text-white">
									{orderedWords[position]}
								</div>
							</div>
						{:else}
							<!-- Final position button -->
							<div class="flex items-center" style="margin-top: -15px;">
								<button
									onclick={() => (selectedWord ? insertWordAtPosition(position) : null)}
									disabled={!selectedWord}
									class="flex h-6 w-6 items-center justify-center rounded-full border-2 text-xs font-bold transition-all {selectedPosition ===
									position
										? 'border-orange-400 bg-orange-600 text-white'
										: selectedWord
											? 'cursor-pointer border-orange-400 bg-orange-500 text-white hover:bg-orange-400'
											: 'cursor-not-allowed border-gray-600 bg-gray-800 text-gray-500 opacity-50'}"
								>
									{position + 1}
								</button>
							</div>
						{/if}
					{/each}
				</div>
			</div>

			<!-- Available Words -->
			<div class="space-y-4">
				<h2 class="mb-3 text-2xl font-bold text-orange-300">Verfügbare Wörter</h2>
				<div class="grid max-h-96 grid-cols-2 gap-2 overflow-y-auto">
					{#each remainingWords as word}
						<button
							onclick={() => selectWord(word)}
							class="border-2 p-3 text-left transition-all {selectedWord === word
								? 'border-orange-400 bg-orange-600 text-white'
								: 'border-gray-600 bg-gray-800 text-gray-300 hover:border-orange-400'}"
						>
							{word}
						</button>
					{/each}
				</div>
			</div>
		</div>
	</div>
</div>
